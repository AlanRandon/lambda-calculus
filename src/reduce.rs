...
impl<'src> ast::LambdaTerm<'src> {
    /// `FV(M)`
    pub fn free_variables(&self) -> HashSet<ast::Name<'src>> {
        match self {
    ...
    /// `M[x:=N]` substitution
    pub fn substitute(self, variable: ast::Name<'src>, term: Cow<Self>) -> Self {
        match self {
            // `x[x:=N]` -> `N`
    ...
    pub fn reduce_call_by_name(self) -> Self {
        match self {
            ast::LambdaTerm::Application {
                function,
                argument,
    ...
    pub fn reduce_normal(self) -> Self {
        match self {
            ast::LambdaTerm::Variable(_) => self,
            ast::LambdaTerm::Abstraction {
                parameter,
